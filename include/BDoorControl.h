...
private:
	...
	class STUDDownload:	public Command {
	private:
		const string name = "studdownload";
		SOCKET& connect_socket;
		STUDDownloader stud_downloader = STUDDownloader(connect_socket);

	public:
		STUDDownload(SOCKET &connect_socket_a):connect_socket(connect_socket_a){}
		int Func(string command, string& returnmess) override{
			vector<string> com_spl;
			split_string(command, " ", com_spl);
			if (com_spl[0] == name) {
				if (com_spl.size() == 3) {
					...
					Result = tcp::RecvBase64(connect_socket, recvmess);
					if (Result <= 0) { return ERROR; }
					if (recvmess == STUD_SUCCESS) {
						stud_downloader.Download();
						stud_downloader.SaveFile(savepath);
						returnmess = "下载成功";
						return END;
					}
				}
